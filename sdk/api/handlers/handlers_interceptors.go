...
func downstreamHeadersAfterInterceptors(baseRaw, finalRaw http.Header, passthrough bool) http.Header {
	...
	if passthrough {
		headers = FilterUpstreamHeaders(finalRaw)
	} else {
		headers = FilterUpstreamHeaders(diffHeaders(baseRaw, finalRaw))
	}
	// Tinker uses these non-sensitive timing headers for its TTFT waterfall.
	// Keep them available even when generic upstream-header passthrough is off.
	for _, key := range []string{"X-Cliproxy-Setup-Ms", "X-Cliproxy-Upstream-Wait-Ms"} {
		if value := finalRaw.Get(key); value != "" {
			if headers == nil {
