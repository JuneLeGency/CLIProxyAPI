...
func TestDownstreamHeadersAlwaysIncludesTinkerTiming(t *testing.T) {
	final := http.Header{
		"Authorization":               []string{"secret"},
		"X-Cliproxy-Setup-Ms":         []string{"12"},
		"X-Cliproxy-Upstream-Wait-Ms": []string{"345"},
	}

	got := downstreamHeadersAfterInterceptors(final, final, false)
	if got.Get("X-Cliproxy-Setup-Ms") != "12" {
		t.Fatalf("setup timing header = %q, want 12", got.Get("X-Cliproxy-Setup-Ms"))
	}
	if got.Get("X-Cliproxy-Upstream-Wait-Ms") != "345" {
		t.Fatalf("upstream timing header = %q, want 345", got.Get("X-Cliproxy-Upstream-Wait-Ms"))
	}
	if got.Get("Authorization") != "" {
		t.Fatal("sensitive upstream header leaked while passthrough is disabled")
	}
}
